...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadBoletos() {
        ...
        .then(sales => {
            ...
            // Atualizar a interface com as somas animadas
            ...
            boletoSales.forEach(sale => {
                ...
                row.innerHTML = `
                    ...
                    <td class="text-center">
                        <button class="btn btn-sm btn-info" onclick="viewSale('${sale.id}')" title="Ver"><i class="fas fa-eye"></i></button>
                        <button class="btn btn-sm btn-primary" onclick="editSale('${sale.id}')" title="Editar"><i class="fas fa-edit"></i></button>
                        <button class="btn btn-sm btn-secondary" onclick="toggleParcelas('${sale.id}')" title="Parcelas"><i class="fas fa-list"></i></button>
                        <button class="btn btn-sm btn-warning" onclick="generateRemessa('${sale.id}')" title="Gerar Remessa"><i class="fas fa-file-export"></i></button>
                    </td>
                `;
                ...
                `;
                boletosTableBody.appendChild(detailRow);
            });
        })
    ...
    function getStatusBadge(status, parcelas) {
        ...
        parcelas.forEach((parcela, index) => {
            ...
            if (parcela.paga) {
                ...
                vencidasCount++;
                badgeClass = 'danger';
                if (Math.abs(daysDiff) >= 15) {  // Alterado para >= 15
                    protestoCount++;
                    statusText = `PROTESTO`;
                } else {
    ...
    // Função para gerar a linha de detalhes das parcelas com ação para marcar como paga
    ...
    // Função atualizada para atualizar a interface com as somas
...
// Função modificada para gerar o arquivo de remessa
